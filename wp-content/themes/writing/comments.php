...
<section id="comments" class="comments-area<?php if (get_option('show_avatars') == 0) { ?> show_no_avatars<?php }?>">
	...
	<?php if ( have_comments() ) : ?>
		<h3 class="comments-title title">
			...
			<span class="screen-reader-text"><?php echo _x('On ', 'After comments number', 'asalah') . get_the_title(); ?></span>
	...
	<?php
		if (is_user_logged_in()) {
			$args = array(
					...
					'must_log_in' =>
'<p class="must-log-in">' .  sprintf( __( 'You must be <a href="%s">logged in</a> to post a comment.', 'asalah' ), wp_login_url( apply_filters( 'the_permalink', get_permalink( ) ) ) ) . '</p>',
