...
<html <?php language_attributes(); ?> class="no-js">
		...
		if (!class_exists('WPSEO_Meta') && (asalah_cross_option('asalah_auto_open_graph') != 'no')) {
		    ...
		    $og_image_url = wp_get_attachment_image_src( get_post_thumbnail_id($post->ID), 'medium' );
				...
				if (asalah_cross_option('asalah_default_share_thumb')) {
					$og_image_url = asalah_cross_option('asalah_default_share_thumb');
				} elseif (asalah_cross_option('asalah_default_logo')) {
						$og_image_url = asalah_cross_option('asalah_default_logo');
				}
			endif;
	...
	// set page conditions
	// sticky menu
	$sticky_menu = asalah_cross_option('asalah_sticky_menu');
	// primary menu exists
	$primary_menu = (has_nav_menu( 'primary' )) ? true : false;

	// sticky logo
	$sticky_logo = asalah_cross_option('asalah_sticky_logo');
	...
	<body <?php body_class($additional_classes); ?>>
		...
		<?php // add facebook script snippet if not disabled
		if (asalah_cross_option('asalah_disable_auto_fb_scripts') !== 'yes') {
		?>
			<!-- Load facebook SDK -->
			<div id="fb-root"></div>
			<script>
			...
			<div class="site_main_container<?php /* add container bg-color classes for content bg */ if (asalah_option('asalah_enable_post_background_color')) {echo ' container bg-color'; } ?>">
				<!-- header -->
					<header class="site_header">
						...
						?>
						<!-- header logo wrapper -->
						<div class="header_logo_wrapper <?php if (asalah_cross_option('asalah_boxed_header')) { echo 'container';} ?> <?php if ($sticky_logo == "yes") { echo 'sticky_logo';}?>">
							<div class="container">
								<?php // logo wrapper
								asalah_header_logo_wrapper(); ?>

								<?php // sliding sidebar icon
								if ( $sliding_icon ) :
