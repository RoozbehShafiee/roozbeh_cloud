...
class postlist_widget extends WP_Widget {
    ...
    function __construct() {
		parent::__construct(
			...
			array(
              'classname' => 'asalah-postlist-widget',
              'description' => '',
              'width' => 250,
              'height' => 350,
    ...
    function widget($args, $instance) {
        ...
        echo asalah_return_blogposts_list($number, array(50, 50), $order, $category, $tags);
        echo '</div>';
        ?>
        <?php
        echo $after_widget;
    ...
    function form($instance) {
        $defaults = array('title' => __('Post List', 'asalah'), 'number' => '3', 'order' => 'date', 'tags' => '', 'category' => '');
        ...
        <?php if (asalah_cross_option('asalah_hits_counter') != 'no') { ?>
                <option value="most_views" <?php if ($instance['order'] == 'most_views') echo "selected=\"selected\"";
        else echo ""; ?>><?php _e('Views Count', 'asalah'); ?></option>
        <?php } ?>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('category'); ?>"><?php _e('Category', 'asalah'); ?>: </label>
            <select id="<?php echo $this->get_field_id('category'); ?>" name="<?php echo $this->get_field_name('category'); ?>" >
              <option value="" <?php if ($instance['category'] == '') echo "selected=\"selected\"";
      else echo ""; ?>><?php _e('All Categories', 'asalah'); ?></option>
