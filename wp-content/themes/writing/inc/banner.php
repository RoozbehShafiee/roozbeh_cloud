...
function asalah_video_frame($prov, $vid) {
    ...
    if ($prov == 'youtube') {
        ?>
          <<?php echo esc_attr($asalah_frame_tag); ?>  class="video_frame <?php echo $lazyclass;?>" <?php echo $src;?>="//www.youtube.com/embed/<?php echo esc_attr($vid); ?>?wmode=transparent&wmode=opaque" frameborder="0" allowfullscreen></<?php echo esc_attr($asalah_frame_tag); ?>>
        <?php
    } elseif ($prov == 'youtu') {
        ?>
        <<?php echo esc_attr($asalah_frame_tag); ?>  class="video_frame <?php echo $lazyclass;?>" <?php echo $src;?>="//www.youtube.com/embed/<?php echo esc_attr($vid); ?>" frameborder="0" allowfullscreen></<?php echo esc_attr($asalah_frame_tag); ?>>
        <?php
    } elseif ($prov == 'vimeo') {
        ?>
        <<?php echo esc_attr($asalah_frame_tag); ?>  class="video_frame <?php echo $lazyclass;?>" <?php echo $src;?>="//player.vimeo.com/video/<?php echo esc_attr($vid); ?>?title=0&amp;byline=0&amp;portrait=0&amp;color=ffffff" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></<?php echo esc_attr($asalah_frame_tag); ?>>
        <?php
    } else {
...
if (!function_exists('asalah_blog_post_banner')) {
function asalah_blog_post_banner($size = "", $class = "") {
    global $asalah_frame_tag;
    global $post;
    global $lazyclass, $src;
    ...
    if ($size == '') {
      if (is_single()) {
        $size = "post-thumbnail";
				if ((asalah_cross_option('asalah_sidebar_position', $post->ID) != 'none') && is_active_sidebar( 'sidebar-1' ) && asalah_option('asalah_banners_devices_size')) {
    ...
    if (get_post_format() == "image" ) {
        ...
        if (is_single()) {
            the_post_thumbnail($size, array("class"=>"img-responsive") );
						...
						if (asalah_option('asalah_banner_single_caption')) {
          ...
          the_post_thumbnail($size, array("class"=>"img-responsive") );
            ...
            echo '</a>';
						...
						if (asalah_option('asalah_banner_blog_caption')) {
							if (get_post(get_post_thumbnail_id())->post_excerpt) { // search for if the image has caption added on it
								echo '<figcaption class="wp-caption wp-caption-text">' . wp_kses_post(get_post(get_post_thumbnail_id())->post_excerpt) . '</figcaption>'; // displays the image caption
							}
						}
      ...
      if (((class_exists( 'Jetpack' )) && (Jetpack::is_module_active( 'carousel' )))) {
        ...
        echo '</div>';
      } elseif (asalah_option('asalah_deactivate_theme_gallery')) {
        echo '<div class="blog_post_banner blog_post_'.get_post_format().''.$class.'">';
        $gallery_shortcode = get_post_meta($post->ID, '_format_gallery_shortcode', true);
        ...
        if (get_post_meta($post->ID, '_format_gallery_shortcode', true) != '' ) {
            ...
            foreach ($attachments as $attachment) {
                ...
                echo '<img src="'.$image_attributes.'"  srcset="'.$thumb_srcset.'" sizes="'.$thumb_sizes.'" alt="'.$attachment_title.'" class="img-responsive">';
                echo '</a>';
        ...
        if (strpos($sound_url, $asalah_frame_tag) != false) {
            ...
            ?>
            <<?php echo esc_attr($asalah_frame_tag); ?> height="166" <?php if ($lazyclass != '') {?>class="lazyload"<?php } ?> style="overflow:hidden;border:none;width:100%" <?php echo $src;?>="https://w.soundcloud.com/player/?url=<?php echo esc_url($sound_url); ?>"></<?php echo esc_attr($asalah_frame_tag); ?>>
            <?php
        }
        echo '</div>';
    } else {
