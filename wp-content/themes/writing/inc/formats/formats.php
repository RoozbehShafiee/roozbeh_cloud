...
function asalah_format_scripts() {
	...
	------------------------------------------- */
	wp_register_script( 'asalah-formatscript', get_template_directory_uri() . '/inc/formats/formatscript.js', array( 'jquery' ), '2', true );
	...
	if (in_array($pagenow, array('post.php', 'post-new.php'))) {
		...
		wp_enqueue_script('asalah-formatscript');
...
function asalah_formats_metaboxes() {
    ...
    // add meta box for commons options in posts and pages
		foreach ($global_types as $type) {
			add_meta_box("asalah_format_meta_boxes", __('Writing Featured Thumbnail', 'asalah'), "asalah_format_meta_boxes", 'post', "side", "low");
		}
}
...
function asalah_post_has_gallery($post_id = null) {
	if (empty($post_id)) {
		$post = get_post();
		$post_id = get_the_id();
	}
