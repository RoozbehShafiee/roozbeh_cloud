...
function asalah_style_customizer_css() {
	...
	if (!(asalah_cross_option('asalah_site_width'))) {
	...
	if (get_theme_mod('asalah_logo_font_type')) {
	...
	if (asalah_option('asalah_enable_body_background_color') == true ) {
		$body_style .= "background-color:".asalah_option('asalah_body_background_color').";";
	}
	...
	if (asalah_option('asalah_enable_main_text_color') == true) {
		$body_font_color = asalah_option('asalah_enable_main_text_color');
	...
	// main font size
	if (intval(asalah_cross_option('asalah_main_font_size')) > 0) {
		$body_style .= "font-size:".asalah_option('asalah_main_font_size')."px;";
	}

	// main line height
	if (intval(asalah_cross_option('asalah_main_line_height')) > 0) {
	...
	// set dropdown menu if custom main bg and no top menu color
	...
	/* End Header Style */
	  ...
	  if ( true == asalah_option('asalah_remove_logo_dot')){
		...
		// tagline font family
		if (get_theme_mod('asalah_tagline_font_type')) {
		...
		// tagline font size
		if (intval(asalah_cross_option('asalah_tagline_font_size')) > 0) {
			$tagline_style .= 'font-size:	'.asalah_option('asalah_tagline_font_size').'px;';
		}

		// tagline line height
		if (intval(asalah_cross_option('asalah_tagline_line_height')) > 0) {
		...
		if (intval(asalah_cross_option('asalah_h1_font_size')) > 0) {
			...
			if (intval(asalah_cross_option('asalah_h2_font_size')) > 0) {
		...
		if (intval(asalah_cross_option('asalah_h1_line_height')) > 0) {
			$h1_size = asalah_option('asalah_h1_line_height');

			$output .= "h1 {line-height:". $h1_size ."px;}";
		}

			if (intval(asalah_cross_option('asalah_h2_line_height')) > 0) {
		...
		foreach ($typography_classes as $class => $class_css) {
			...
			if ((get_theme_mod('asalah_'.$class.'_font_type')) || (asalah_option('asalah_'.$class.'_font_size')) || (asalah_option('asalah_'.$class.'_line_height'))) {
				$output .= $class_css." {";

					if ( get_theme_mod('asalah_'.$class.'_font_type')) {
      ...
      $output .= "}";
		...
		if ((asalah_option('asalah_post_with_formatting') == 'yes') && (asalah_cross_option('asalah_post_excerpt') != "disabled")) {
