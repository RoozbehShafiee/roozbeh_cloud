...
/*
	This will clean mediaelement default skin from footer action 
	And will add it to header to clear any conflict with theme style
*/
...
function feather_clear_meskin(){
	 wp_dequeue_style('mediaelement');
	 wp_dequeue_style('wp-mediaelement');
}
add_action('wp_footer' , 'feather_clear_meskin');
...
function feather_addons()
{
	...
	$bg_slider_speed = isset($feather_theme_options['bg_slider_speed']) ? $feather_theme_options['bg_slider_speed'] : '5000';
	$more_color = isset($feather_theme_options['body_link_color']) ? $feather_theme_options['body_link_color'] : '#f4793d';
	$more_hover_color = isset($feather_theme_options['body_link_hover_color']) ? $feather_theme_options['body_link_hover_color'] : '#636467';
    ...
    wp_register_script('jquery-isotope-min' , get_template_directory_uri() . '/js/isotope.min.js' , '' , false , true);
    ...
    wp_localize_script('custom-js' , 'feather' , array(
        ...
        'bg_slider_speed' => $bg_slider_speed,
        'more_color' => $more_color ,
        'more_hover_color' => $more_hover_color
    ));
	...
	wp_register_style('feather-default-fonts' , 'https://fonts.googleapis.com/css?family=Lato:400,300,700,400italic,900|Roboto+Slab:400,300,700|Voltaire');

    wp_register_style('feather-bootstrap' , get_template_directory_uri() . '/css/bootstrap.min.css');
    wp_register_style('feather-css' , get_template_directory_uri(). '/css/main.css' , array('mediaelement' , 'wp-mediaelement'));
...
function feather_disqus($id , $title){
	global $feather_theme_options;
	// check if disqus shortname is set in theme options
	if(isset($feather_theme_options['disqus_shortname']) && $feather_theme_options['disqus_shortname'] != '')
	{		
			// prepare disqus url
			$name = $feather_theme_options['disqus_shortname'];
			$url = 'http://' . $name . '.disqus.com/embed.js';

			// enqueue script
			wp_enqueue_script('feather_disqus' , $url);
            ...
            <div class="post-inner-content secondary-content-box secondary-comments-section related-posts-wrapper">
...
/* -------------------------------------------------------------- 
 	Facebook Comments
-------------------------------------------------------------- */
function feather_facebook_comments($url){
		global $feather_theme_options;
		$count = (isset($feather_theme_options['facebook_comments_count'])) ? $feather_theme_options['facebook_comments_count'] : '5';
		$theme = (isset($feather_theme_options['facebook_comments_theme'])) ? $feather_theme_options['facebook_comments_theme'] : 'light';
		$app_id = (isset($feather_theme_options['facebook_app_id'])) ? $feather_theme_options['facebook_app_id'] : '';
		if($app_id != '')
		{
			...
			$return .= '<div class="fb-comments" data-href="'.$url.'" data-numposts="'.$count.'" data-colorscheme="'.$theme.'"></div>';
...
function feather_content_more( $link ) {
	...
	if(isset($feather_theme_options) && $feather_theme_options['enable_readmore'] != 0) {
			...
			if($link != '')
			{
					...
					if(is_page_template('page-blog-grid.php') || is_page_template('page-blog-gridalt.php'))
					{
						$link = '<a class="feather-more-link" href="'.get_permalink().'">'.$readmore_text.'</a>';
					}
					else{
...
/* -------------------------------------------------------------- 
 	Date Function
-------------------------------------------------------------- */
function feather_date(){
	global $feather_theme_options;
	if(isset($feather_theme_options['edit_date_format']) && $feather_theme_options['edit_date_format'] != '')
	{
		return get_the_date(  $feather_theme_options['edit_date_format']  );
	}
	else{
